...
def test_cpm_records_bounds_priors_and_restarts(store):
    ...
    parameters = Parameters(alpha=Value(value=0.1, lower=0.0, upper=1.0))
    ...
    fit = FminBound(
        ...
        ppt_identifier="ppt", number_of_starts=2,
    )
    ...
    # Restart count lives on the optimiser, not in kwargs. With >1 start and no
    # explicit guess, initial guesses are random -- the run is not reproducible
    # and the manifest must say so.
    assert m.get("param.fit.number_of_starts") == "2"
    assert m.get("param.fit.initial_guess_supplied") == "false"
